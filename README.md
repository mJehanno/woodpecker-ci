...
```shell
  docker buildx build -t mjehanno/woodpecker-ci:latest . --load
```

To install the extension, use `make install-extension` **or**:

```shell
  docker extension install mjehanno/woodpecker-ci:latest
```
...
```shell
  docker extension dev ui-source mjehanno/woodpecker-ci:latest http://localhost:3000
```

In order to open the Chrome Dev Tools for your extension when you click on the extension tab, run:

```shell
  docker extension dev debug mjehanno/woodpecker-ci:latest
```

Each subsequent click on the extension tab will also open Chrome Dev Tools. To stop this behaviour, run:

```shell
  docker extension dev reset mjehanno/woodpecker-ci:latest
```
...
```shell
docker extension update mjehanno/woodpecker-ci:latest
```
...
```shell
docker extension rm mjehanno/woodpecker-ci:latest
```
